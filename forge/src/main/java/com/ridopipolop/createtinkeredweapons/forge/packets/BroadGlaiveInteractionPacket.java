package com.ridopipolop.createtinkeredweapons.forge.packets;
...
public class BroadGlaiveInteractionPacket extends SimplePacketBase {
  private InteractionHand interactionHand;
  private int target;
  private Vec3 specificPoint;

  public BroadGlaiveInteractionPacket(Entity target) {
    this(target, null);
  }

  public BroadGlaiveInteractionPacket(Entity target, InteractionHand hand) {
    this(target, hand, null);
  }

  public BroadGlaiveInteractionPacket(Entity target, InteractionHand hand, Vec3 specificPoint) {
    interactionHand = hand;
    this.specificPoint = specificPoint;
    this.target = target.getId();
  }

  public BroadGlaiveInteractionPacket(FriendlyByteBuf buffer) {
  ...
  public boolean handle(Context context) {
    context.enqueueWork(() -> {
      ...
      if (entityByID != null && ModItems.BROAD_GLAIVE.isIn(sender.getMainHandItem())) {
        double d = sender.getAttribute(ForgeMod.ENTITY_REACH.get()).getValue();
